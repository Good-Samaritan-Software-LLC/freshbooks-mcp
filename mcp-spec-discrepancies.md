...
| B12 | breaking | `src/tools/invoice/schemas.ts` → `InvoiceUpdateInputSchema.status` (string enum) passed through verbatim by `invoice-update.ts` | `#/components/schemas/InvoiceWrite/properties/status` | **LIVE-VERIFIED** (`scripts/probe-invoice-status.mjs` / `-map.mjs`): PUT `status:"sent"` → 422 errno 1015 "The field 'status' must be a number."; PUT `status:2` → 200; fresh-fixture sweep 0–8 → only 0/1/2/3 accepted | Invoice `status` is written (and read) as the SDK's string enum value. | The wire `status` is **numeric** on read AND write (0 disputed, 1 draft, 2 sent, 3 viewed; 4–8 payment-driven, rejected on write). Every `invoice_update` status change has always 422'd — and complying with the API error by sending `2` is then rejected by the MCP's own string-enum schema: a hard contradiction trap. Reads have always returned numbers despite the string output contract. | Map name→number in the handler (disputed=0, draft=1, sent=2, viewed=3), narrow the update input enum to the four settable values, annotate output `status` as numeric. **FIXED in this PR** (found via live use, post-#86). |
| B13 | breaking | `src/tools/invoice/invoice-create.ts` / `invoice-update.ts` → `payload.dueDate = input.dueDate` | `#/components/schemas/InvoiceWrite` (`due_offset_days`) | **LIVE-VERIFIED** (`scripts/probe-invoice-duedate.mjs`): POST/PUT `due_date` → **403 errno 1038** "Write access denied ... field due_date"; `due_offset_days: 30` → 200 on both create AND update, due_date derived = create_date + 30. Also: SDK `transformInvoiceRequest` (`Invoices.js:196-198`) has **no due_date mapping** — only `due_offset_days` | Setting `dueDate` on invoice create/update sets the invoice's due date. | Double failure: (1) the SDK transform silently DROPS `dueDate` (never reaches the wire), and (2) even raw, `due_date` is **read-only** — the API derives it from `create_date + due_offset_days` (same design as bills, #80). So the MCP's `dueDate` input has always been a **silent no-op**. | Derive `dueOffsetDays = daysBetween(createDate, dueDate)` and send that (create: default base = today; update: fetch the invoice's `create_date` when `createDate` isn't provided); expose `dueOffsetDays` directly as an alternative input; mark `due_date` readOnly in the spec. **FIXED in this PR.** |
