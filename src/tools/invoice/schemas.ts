...
export const InvoiceSchema = z.object({
  ...
  dueOffsetDays: z.number().optional().describe('Days after the invoice date the payment is due (the writable source of dueDate)'),
...
export const InvoiceCreateInputSchema = z.object({
  ...
  dueDate: z.string().optional().describe('Payment due date (YYYY-MM-DD). The API stores due dates as due_offset_days relative to the invoice date; the offset is derived automatically (due_date itself is read-only on the wire)'),
  dueOffsetDays: z.number().int().min(0).optional().describe('Days after the invoice date the payment is due (alternative to dueDate; wins if both are given)'),
...
export const InvoiceUpdateInputSchema = z.object({
  ...
  dueDate: z.string().optional().describe('Payment due date (YYYY-MM-DD). Stored by the API as due_offset_days relative to the invoice date; the offset is derived automatically (reads the invoice first when createDate is not also provided)'),
  dueOffsetDays: z.number().int().min(0).optional().describe('Days after the invoice date the payment is due (alternative to dueDate; wins if both are given)'),
