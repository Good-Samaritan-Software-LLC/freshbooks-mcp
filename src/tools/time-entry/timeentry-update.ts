...
→ billable: false`,
  ...
  async execute(
      ...
      async (
        ...
        logger.info('Time entry updated successfully', {
          ...
          updatedFields: Object.keys(input).filter(
            (k) => k !== 'accountId' && k !== 'timeEntryId' && input[k as keyof typeof input] !== undefined
          ),
        });
    ...
    return handler(input, { accountId: input.accountId, entityId: input.timeEntryId }) as any;
  },
};
