  ...
  async execute(
    ...
    const handler = ErrorHandler.wrapHandler(
      ...
      async (input: z.infer<typeof TaskUpdateInputSchema>, _context: ToolContext) => {
        const { businessId, taskId, name, description, billable, rate, visState } = input;
        ...
        const result = await client.executeWithRetry('task_update', async (fbClient) => {
          ...
          const response = await fbClient.tasks.update(updateData as any, String(businessId), taskId);
