  ...
  async execute(
    ...
    const handler = ErrorHandler.wrapHandler(
      ...
      async (input: z.infer<typeof TaskCreateInputSchema>, _context: ToolContext) => {
        const { businessId, name, description, billable, rate } = input;
        ...
        const result = await client.executeWithRetry('task_create', async (fbClient) => {
          ...
          const response = await fbClient.tasks.create(taskData as any, String(businessId));
