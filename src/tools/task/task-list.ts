...
import { z } from 'zod';
import { BusinessIdSchema, PaginationSchema } from '../base-tool.js';
import { TaskListOutputSchema } from './schemas.js';
import { ErrorHandler } from '../../errors/error-handler.js';
import { ToolContext } from '../../errors/types.js';
import { FreshBooksClientWrapper } from '../../client/index.js';

const InputSchema = BusinessIdSchema.extend({
  ...
  async execute(
    ...
    const handler = ErrorHandler.wrapHandler(
      ...
      async (input: z.infer<typeof InputSchema>, _context: ToolContext) => {
        const { businessId, page, perPage } = input;
        ...
        const result = await client.executeWithRetry('task_list', async (fbClient) => {
          ...
          // Call FreshBooks API
          const response = await fbClient.tasks.list(String(businessId), queryBuilders);
